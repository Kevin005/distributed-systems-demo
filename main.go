...
func main() {
	...
	fmt.Println("my node info:", me.String())
...
func connectToCluster(me NodeInfo, dest NodeInfo) bool {
	...
	if err != nil {
		...
		json.NewEncoder(connOut).Encode(&requestMessage) //编码数据结构，数据结构转io流

		decoder := json.NewDecoder(connOut) //解码流，把流解为数据结构
